...
async function handler(req) {
  ...
  if (action === "analytics-track" && req.method === "POST") {
    ...
    if (b.type === "pageview") {
      ...
      existing.events.push({ t: b.type, page: clampStr(b.page, 100), target: clampStr(b.target, 100), at: new Date().toISOString() });
      if (existing.events.length > 1000) existing.events = existing.events.slice(-1000);
    } else if (b.type === "error") {
      existing.events.push({ t: "error", page: clampStr(b.page, 100), msg: clampStr(b.message, 200), at: new Date().toISOString() });
      if (existing.events.length > 1000) existing.events = existing.events.slice(-1000);
    }
